...
The second language-specific pass — Japanese — is in as well, following the same layered-on-top pattern rather than being baked into the core: two on-screen keyboards (`JapaneseKanaKeyboard`, a JIS kana layout, and `JapanesePhoneticKeyboard`, a romaji IME built on `domain/japanese/romajiToKana.ts` with a candidate picker for kana/kanji conversion), a per-device furigana toggle over kanji, and a keyboard input-method choice (kana vs. phonetic) alongside Persian's own layout-vs-phonetic one. Both languages share the hover/focus word tooltip: any native-script word matching a lexeme can reveal its romanization and/or its English gloss, each independently switchable from the in-lesson settings cog (`showRomanizationHints`, `showTranslationHints` — local per device, like furigana).

Around the lesson core: a first-run onboarding flow (pick a language, then a short tutorial) for accounts enrolled in nothing yet; checkpoints/test-outs (`/checkpoint/:unitKey/:skillKey`) for proving a position's material is already known instead of taking every lesson in it; practice mode, so a lesson with nothing currently due can still be redone (local recap only — no FSRS credit, and never a hard lockout); a header course switcher for learners enrolled in more than one language, with the catalog behind it for adding another; and a per-exercise "report a problem" control that posts to the backend's exercise-report endpoint when content looks wrong.

Not yet built: a `special`/misc skill category (content outside the normal progression, e.g. slang or idioms) — the schema's `category` is a fixed enum today, but the path/list rendering built for story/conversation/song is already category-agnostic below the standard/non-standard split, so a new value there would need no new client-side unlock logic, just a schema value, a theme icon, and content.
